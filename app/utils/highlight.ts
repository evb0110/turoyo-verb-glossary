...
const isSupported = () => typeof window !== 'undefined' && 'CSS' in window && 'highlights' in (CSS as any)

export function setHighlights(name: string, root: ParentNode, ranges: Array<{ startNode: Node, startOffset: number, endNode: Node, endOffset: number }>) {
  if (!isSupported()) return

  try {
    // Remove existing highlight with the same name
    const existing = (CSS as any).highlights.get(name)
    if (existing) (CSS as any).highlights.delete(name)
    ...
    // Silently ignore if constructing ranges fails
  }
}
...
export function clearHighlights(name: string) {
  if (!isSupported()) return
  try {
    if ((CSS as any).highlights.has(name)) {
